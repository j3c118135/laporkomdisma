...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            ...
            <form method="POST" action="<?php echo base_url('admin/komdisma/simpan'); ?>" class="needs-validation" novalidate>
                <?= csrf_field();?>
            ...
            <form method="post" action="<?= base_url('admin/akun/hapus');?>" id="form-delete">
                <div class="table-responsive">
                    <table class="table table-striped" id="tableUser" width="100%" cellspacing="0">
                        ...
                        <tbody>
                        <?php foreach ($komdismas as $komdisma) : ?>
                            <tr>
                                <td></td>
                                <td><?= $komdisma->id;?></td>
                                <td><?= $komdisma->nama;?></td>
                                <td>
                                    <?php if($komdisma->role != "Super Admin") { ?>
                                    <form action="<?= base_url('admin/komdisma/role');?>" method="post"> 
                                        <input type="hidden" value="<?= $komdisma->id_akun;?>" name="id">
                                        <button type="submit" class="btn rounded-circle btn-outline-primary btn-sm" data-toggle="tooltip" data-placement="top" title="Jadikan Super Admin" id="UbahRole"><i class="fas fa-pen"></i></button>
                                    </form>
                                    <?php } else { ?>
                                        <button class="btn rounded-circle btn-outline-primary btn-sm" disabled><i class="fas fa-pen"></i></button>
                                    <?php }?>
                                </td>
                                <td><div class="custom-control custom-checkbox">
                                <input type="checkbox" class="custom-control-input check-item" name='id[]' value="<?=$komdisma->id_akun?>" id="checkitem<?=$komdisma->id_akun?>">
                                <label class="custom-control-label" for="checkitem<?=$komdisma->id_akun?>"></label>
                                </div></td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="<?= base_url('admin/komdisma/import');?>" method="POST" class="needs-validation" novalidate enctype="multipart/form-data">
